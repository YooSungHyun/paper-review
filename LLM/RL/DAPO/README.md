...
GRPO를 보면, 각 샘플내 토큰 로스에 대한 평균을 구하고, 샘플간 손실을 집계하는 과정을 거친다.
...
크로스엔트로피 이슈를 해결할때 그랬듯이, 토큰별 loss를 합하고 길이로 나눈다. (길이정규화)

그다음에 배치평균을 구하는 것으로 해결함.
